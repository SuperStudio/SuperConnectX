...
describe('useTabManager', () => {
  ...
  describe('getConnectionStatus', () => {
    ...
    it('should use the AI session snapshot for managed Telnet and FTP tabs', () => {
      const manager = useTabManager(refs.com, refs.telnet)
      const telnet = makeTab({ aiManaged: true, aiSessionState: 'connected' })
      const ftp = makeFtpTab({ aiManaged: true, aiSessionState: 'closed' })

      expect(manager.getConnectionStatus(telnet)).toBe('connected')
      expect(manager.getConnectionStatus(ftp)).toBe('disconnected')
    })
  })
  ...
  describe('ensureAiSessionTab', () => {
    ...
    it('should keep reserved AI tab fields when desired config contains storage identifiers', () => {
      const manager = useTabManager(refs.com, refs.telnet)
      const tab = manager.ensureAiSessionTab({
        sessionId: 'mcp-ai-telnet',
        state: 'connected',
        connectionType: 'telnet',
        name: 'AI Telnet',
        host: '127.0.0.1',
      ...
      expect(tab).not.toHaveProperty('wasConnected')
    })

    it('should close a tab even when its stored id is numeric and the UI emits a string id', async () => {
      const cleanup = vi.fn()
      refs.telnet = { '42': { cleanup } }
      const manager = useTabManager(refs.com, refs.telnet)
      manager.connectionTabs.value = [
        makeTab({ id: 42 as unknown as string, sessionId: 'numeric-tab-session' })
      ]

      await manager.closeTabOnly('42')

      expect(cleanup).toHaveBeenCalled()
      expect(manager.connectionTabs.value).toHaveLength(0)
      expect(mockStopConnect).toHaveBeenCalledWith(
        expect.objectContaining({ sessionId: 'numeric-tab-session' })
      )
    })
  ...
  describe('openAiServiceTab', () => {
    it('should normalize a restored legacy AI tab title', () => {
      const manager = useTabManager(refs.com, refs.telnet)
      manager.connectionTabs.value = [makeTab({
        id: 'ai-service',
      ...
      manager.openAiServiceTab('overview')

      expect(manager.connectionTabs.value).toHaveLength(1)
      expect(manager.connectionTabs.value[0].name).toBe('AI 交互桥梁')
    })
  })
})
