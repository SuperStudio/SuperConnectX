...
export interface AiBridgeActivityLogInfo {
  filePath: string
  directory: string
}

export interface AiBridgeActivityLogger {
  error: (message: string, meta?: unknown) => void
  warn?: (message: string, meta?: unknown) => void
}

export interface AiBridgeActivityLogOptions {
  directory?: string
  maxFileBytes?: number
  maxFiles?: number
}
...
function normalizeOptions(
  options: AiBridgeActivityLogOptions,
...
/**
 * 将脱敏后的 `ai.activity` 事件异步写入有界 NDJSON 日志。
 * 单文件达到上限后按编号轮转；历史读取只扫描各文件尾部的有限字节。
 */
export default class AiBridgeActivityLog {
  ...
  constructor(
    private readonly logger: AiBridgeActivityLogger,
    options: AiBridgeActivityLogOptions = {}
  ) {
  ...
  configure(options: AiBridgeActivityLogOptions): void {
    ...
    this.writeChain = this.writeChain
      ...
      .catch((error) => this.logger.error('[AiBridgeActivityLog] failed to apply limits', error))
  }
  ...
  record(event: RuntimeEvent): void {
    ...
    if (bytes > this.options.maxFileBytes || this.pendingBytes + bytes > MAX_PENDING_BYTES) {
      if (!this.queueOverflowWarned) {
        ...
        this.logger.warn?.('[AiBridgeActivityLog] activity queue limit reached; entries dropped')
      }
      return
    }
  ...
  async readHistory(limit: number = DEFAULT_HISTORY_LIMIT): Promise<RuntimeEvent[]> {
    ...
    try {
      ...
      this.logger.error('[AiBridgeActivityLog] failed to read activity', error)
      return []
    }
  }

  getInfo(): AiBridgeActivityLogInfo {
  ...
  private drainPending(): void {
    ...
    this.writeChain = this.writeChain
      ...
      .catch((error) => this.logger.error('[AiBridgeActivityLog] failed to append activity', error))
  }
